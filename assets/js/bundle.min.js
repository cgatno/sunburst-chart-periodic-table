...
// event handlers, etc.
c.getChartDataSource(function(h){// Initialize the property tile by loading it into its module
let j=new g(document.getElementById("properties-tile"));window.onresize=function(){j.centerInParent()};let k=new wijmo.chart.hierarchical.Sunburst("#periodic-sunburst");// Let the Sunburst Chart know we're going to start making changes
k.beginUpdate(),k.legend.position="None",k.innerRadius=0.3,k.selectionMode="Point",k.dataLabel.position="Center",k.dataLabel.content="{name}",k.itemsSource=h,k.binding="value",k.bindingName=["groupName","subGroupName","eleSymbol"],k.childItemsPath=["subGroups","elements"],k.hostElement.addEventListener("click",function(m){let p=k.hitTest(m.pageX,m.pageY);j.showInfoPanel(d.mapChartNameToObject(p.name,k.collectionView))}),k.endUpdate(),console.log("Sunburst chart initialized:"),console.log(k)})},{"./lib/data/DAL/DataSourceAdapter":2,"./lib/view/PropertiesTile":7,"./lib/view/ViewAdapter":8}],2:[function(b,c){"use strict";/**

(function b(c,d,g){function h(m,p){if(!d[m]){if(!c[m]){var q="function"==typeof require&&require;if(!p&&q)return q(m,!0);if(j)return j(m,!0);var v=new Error("Cannot find module '"+m+"'");throw v.code="MODULE_NOT_FOUND",v}var w=d[m]={exports:{}};c[m][0].call(w.exports,function(x){var y=c[m][1][x];return h(y?y:x)},w,w.exports,b,c,d,g)}return d[m].exports}var j="function"==typeof require&&require;for(var k=0;k<g.length;k++)h(g[k]);return h})({1:[function(b){"use strict";function c(j){let k=j.clientX,m=j.clientY,p=h.hitTest(k,m);console.log("Gathered HitTestInfo for click:"),console.log(p)}var d=b("./lib/data/DAL/DataSourceAdapter");// Hook up plain old DOM event handlers
let g=document.getElementById("periodic-sunburst");g.addEventListener?g.addEventListener("click",c,!1):g.attachEvent&&g.attachEvent("click",c);// Declare the sunburst object in the global scope so we can reference it in
// event handlers, etc.
let h;d.getChartDataSource(function(j){h=new wijmo.chart.hierarchical.Sunburst("#periodic-sunburst"),h.beginUpdate(),h.legend.position="None",h.innerRadius=0.3,h.selectionMode="Point",h.dataLabel.position="Center",h.dataLabel.content="{name}",h.itemsSource=j,h.binding="value",h.bindingName=["groupName","subGroupName","eleSymbol"],h.childItemsPath=["subGroups","elements"],h.endUpdate(),console.log("Sunburst chart initialized:"),console.log(h)})},{"./lib/data/DAL/DataSourceAdapter":2}],2:[function(b,c){"use strict";/**
...
for(let E=0;E<p.length;E++)"Metal"===p[E]?B.subGroups.push(new h("Others")):B.subGroups.push(new h(p[E])),B.subGroups[E].characteristics=w[E];let C=new j("Non Metals");// Add all of the nonmetal subGroups
for(let E=0;E<q.length;E++)"Nonmetal"===q[E]?C.subGroups.push(new h("Others")):C.subGroups.push(new h(q[E])),C.subGroups[E].characteristics=x[E];let D=new j("Others");// Add all of the other subGroups

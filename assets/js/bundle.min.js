...
let w=m.hitTest(v.pageX,v.pageY);k.showInfoPanel(d.getObjectFromChartName(w.name,m.collectionView))});// Set selected element variables in this outer scope to preserve them
 ...
 */function d(z){return z.subGroupName===this.type+"s"}var g=b("../model/Element"),h=b("../model/SubGroup"),j=b("../model/Group"),k=b("../../util/JsonDataLoader"),m=function(){};// Define constants that we need to properly group elements
...
for(let E=0;E<p.length;E++)"Metal"===p[E]?B.subGroups.push(new h("Other Metals")):B.subGroups.push(new h(p[E]+"s")),B.subGroups[E].characteristics=w[E];let C=new j("Nonmetals");// Add all of the nonmetal subGroups
for(let E=0;E<q.length;E++)"Nonmetal"===q[E]?C.subGroups.push(new h("Other Nonmetals")):C.subGroups.push(new h(q[E]+"s")),C.subGroups[E].characteristics=x[E];let D=new j("Others");// Add all of the other subGroups
for(let E=0;E<v.length;E++)D.subGroups.push(new h(v[E]+"s")),D.subGroups[E].characteristics=y[E];// Retrieve an array listing of all elements

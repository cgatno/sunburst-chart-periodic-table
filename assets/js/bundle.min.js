...
// event handlers, etc.
c.getChartDataSource(function(h){function j(v,w){let x=document.elementFromPoint(v,w);if("undefined"!=typeof x&&"text"===x.tagName)return j(v,w+0.05),!1;if("undefined"!=typeof p&&"undefined"!=typeof q&&p.setAttribute("fill",q),"undefined"!=typeof x&&x.hasAttribute("fill")&&"path"===x.tagName){p=x;let y=x.getAttribute("fill");q=y;let z=y.replace("rgba(","").replace(")","").split(",");z[3]="1.0",x.setAttribute("fill","rgba("+z.join(",")+")")}}// Let the chart know that everything is updated now
...
m.beginUpdate(),m.legend.position="None",m.innerRadius=0.3,m.selectionMode="Point",m.dataLabel.position="Center",m.dataLabel.content="{name}",m.itemsSource=h,m.binding="value",m.bindingName=["groupName","subGroupName","eleSymbol"],m.childItemsPath=["subGroups","elements"],m.hostElement.addEventListener("click",function(v){j(v.pageX,v.pageY);let w=m.hitTest(v.pageX,v.pageY);k.showInfoPanel(d.mapChartNameToObject(w.name,m.collectionView))});let p,q;m.endUpdate()})},{"./lib/data/DAL/DataSourceAdapter":2,"./lib/view/PropertiesTile":7,"./lib/view/ViewAdapter":8}],2:[function(b,c){"use strict";/**

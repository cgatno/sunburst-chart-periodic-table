...
c.getChartDataSource(function(h){/**
     ...
     */function j(v,w){"undefined"!=typeof p&&"undefined"!=typeof q&&p.setAttribute("fill",q);// Define our selected element and check to see if it's a panel that we can fill
let x=document.elementFromPoint(v,w);if("undefined"!=typeof x&&x.hasAttribute("fill")&&"path"===x.tagName){p=x;let y=x.getAttribute("fill");q=y;let z=y.replace("rgba(","").replace(")","").split(",");z[3]="1.0",x.setAttribute("fill","rgba("+z.join(",")+")")}// TODO: provide a solution for when text within a panel is clicked before releasing v0.1.0
...
m.beginUpdate(),m.legend.position="None",m.innerRadius=0.3,m.selectionMode="Point",m.dataLabel.position="Center",m.dataLabel.content="{name}",m.itemsSource=h,m.binding="value",m.bindingName=["groupName","subGroupName","eleSymbol"],m.childItemsPath=["subGroups","elements"],m.hostElement.addEventListener("click",function(v){j(v.pageX,v.pageY);// Perform a hit test to get a clicked panel's name then use it to set up the info panel via the ViewAdapter
let w=m.hitTest(v.pageX,v.pageY);k.showInfoPanel(d.mapChartNameToObject(w.name,m.collectionView))});// Set selected element variables in this outer scope to preserve them
let p,q;m.endUpdate()})},{"./lib/data/DAL/DataSourceAdapter":2,"./lib/view/PropertiesTile":7,"./lib/view/ViewAdapter":8}],2:[function(b,c){"use strict";/**
 ...
 * 
 */// Pull in dependencies
var d=b("../data/model/Element"),g=b("../data/model/SubGroup"),h=b("../data/model/Group"),j=function(m){this.domElement=m,this.centerInParent()};j.prototype.centerInParent=function(){let m=this.domElement,p=m.parentElement;m.style.top=p.offsetHeight/2-m.offsetHeight/2+"px",m.style.left=p.offsetWidth/2-m.offsetWidth/2+"px"},j.prototype.show=function(){let m=this.domElement;m.style.visibility="visible"},j.prototype.hide=function(){let m=this.domElement;m.style.visibility="hidden"};const k="<br>";j.prototype.showInfoPanel=function(m){if("undefined"==typeof m)this.hide();else{// the user selected something on the chart, display the appropriate info
...
if(p.style.display="none",v.style.display="none",q.style.display="none",m instanceof h){document.getElementById("group-name").innerText=m.groupName,document.getElementById("subgroup-listing").innerHTML="";for(let w=0;w<m.subGroups.length;w++)document.getElementById("subgroup-listing").innerHTML+=m.subGroups[w].subGroupName,document.getElementById("subgroup-listing").innerHTML+=" ",document.getElementById("subgroup-listing").innerHTML+="("+m.subGroups[w].elements.length+")",document.getElementById("subgroup-listing").innerHTML+=k;p.style.display="block"}else if(m instanceof g){document.getElementById("subGroup-name").innerText=m.subGroupName,document.getElementById("num-elements").innerText=m.elements.length,document.getElementById("characteristic-listing").innerHTML="";let w=m.characteristics.split(",");for(let x=0;x<w.length;x++)document.getElementById("characteristic-listing").innerHTML+=w[x],document.getElementById("characteristic-listing").innerHTML+=k;v.style.display="block"}else m instanceof d&&(document.getElementById("element-symbol").innerText=m.eleSymbol,document.getElementById("element-name").innerText=m.elementName,document.getElementById("atomic-number").innerText=m.atomicNumber,document.getElementById("atomic-weight").innerText=(+m.atomicWeight).toFixed(2),q.style.display="block");this.centerInParent(),this.show()}},c.exports=j},{"../data/model/Element":3,"../data/model/Group":4,"../data/model/SubGroup":5}],8:[function(b,c){"use strict";/**
 * ViewAdapter.js
 * 
 * Provides some helper methods for handling Sunburst Chart events and generating the "property tiles"
 * dynamically based on chart selections.
